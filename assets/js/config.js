...
(function () {
  ...
  function applyBranding() { document.title = document.title.replaceAll("StudentBnB", "CasaStudent"); const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT); let node; while ((node = walker.nextNode())) node.nodeValue = node.nodeValue.replaceAll("StudentBnB", "CasaStudent"); document.querySelectorAll("[aria-label]").forEach(element => element.setAttribute("aria-label", element.getAttribute("aria-label").replaceAll("StudentBnB", "CasaStudent"))); document.querySelectorAll(".brand").forEach(brand => { const labels = [...brand.children].filter(element => element.tagName === "SPAN" && !element.classList.contains("brand-icon")); const label = labels[labels.length - 1]; if (!label) return; const small = label.querySelector("small"); const smallHtml = small ? small.outerHTML : ""; label.innerHTML = `Casa<strong>Student</strong>${smallHtml}`; }); }
  function apply() {
    applyBranding();
